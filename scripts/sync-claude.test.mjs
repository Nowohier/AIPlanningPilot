...
describe('writeProjectConfig', () => {
    ...
    it('writes project.json with project name from path', () => {
        ...
        assert.equal(config.projectName, 'ExampleProject');
    });
    ...
    it('handles trailing slashes', () => {
        ...
        writeProjectConfig(planningRepo, 'D:\\Projects\\myapp\\');
    ...
    it('skips write when unchanged', () => {
        ...
        writeProjectConfig(planningRepo, 'M:\\Projects\\ExampleProject');
        const result = writeProjectConfig(planningRepo, 'M:\\Projects\\ExampleProject');

        assert.equal(result.written, false);
    });

    it('overwrites when project name changes', () => {
        const planningRepo = join(tempDir, 'planning');
        mkdirSync(join(planningRepo, 'main'), { recursive: true });

        writeProjectConfig(planningRepo, 'M:\\Projects\\old-project');
        const result = writeProjectConfig(planningRepo, 'M:\\Projects\\new-project');

        assert.equal(result.written, true);
        const config = JSON.parse(readFileSync(join(planningRepo, 'main', 'project.json'), 'utf-8'));
        assert.equal(config.projectName, 'new-project');
    });

    it('creates main directory if missing', () => {
        const planningRepo = join(tempDir, 'planning');

        writeProjectConfig(planningRepo, 'D:\\Projects\\myapp');

        assert.ok(existsSync(join(planningRepo, 'main', 'project.json')));
    });
});
...
`, 'utf-8');
    ...
    it('performs full end-to-end sync', () => {
        ...
        // Project config
        assert.equal(result.projectConfig.written, true);
        const projectConfig = JSON.parse(readFileSync(join(planningDir, 'main', 'project.json'), 'utf-8'));
        assert.equal(projectConfig.projectName, 'MyProject');
    });
    ...
    it('is idempotent (second run produces no changes)', () => {
        ...
        assert.equal(result.projectConfig.written, false);
