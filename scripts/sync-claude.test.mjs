...
describe('runSync (integration)', () => {
  ...
  beforeEach(() => {
    ...
    // Set up .claude/CLAUDE.md
    writeClaudeMd(tempDir, {
      developer: 'TestDev',
      projectRepo: 'D:\\MyProject',
      planningRepo: 'D:\\MyProject\\restructuring',
    });

    // Set up claude-backup structure
    const backupBase = join(tempDir, 'restructuring', 'claude-backup');
...
`, 'utf-8');
  ...
  it('performs full end-to-end sync', () => {
    ...
    assert.ok(cmd.includes('D:\\MyProject\\restructuring\\main\\STATE.md'));
    ...
    assert.ok(envSh.includes('PLANNING_REPO="/d/MyProject/restructuring"'));
